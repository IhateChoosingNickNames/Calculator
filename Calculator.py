...
def is_valid(calculation_string, n):
    ...
    for i in calculation_string:
        if i not in '01234!56789-&%.+/)(* ' or calculation_string.count('(') != calculation_string.count(')') or \
                calculation_string.strip()[-1] in '-+*&%^/' or calculation_string.strip()[0] in '+/%&*^':
            return False
    return True
...
def list_calculation_creator(calculation_string):
    ...
    # скобок
    calculation_string_replaced = calculation_string.strip().replace(' ', '').replace('**', '^').replace(')(',
                                                                                                         ')*(').replace(
        '*-', '*-1*').replace('/-', '*-1/').replace('+-', '-').replace('//', '&').replace('--', '+').replace('-(', '+0-(')
    list_of_calculation = []
    j = -1
    for i in range(len(calculation_string_replaced)):
        ...
        elif calculation_string_replaced[i] in '+-!*&%/^' and len(list_of_calculation) > 0 and list_of_calculation[
            -1] not in '-+*%&/^(':
            list_of_calculation.append(calculation_string_replaced[i])
...
def main(calculation_string, n):
    ...
    if is_valid(calculation_string, n):
        ...
        list_of_calculation = list_calculation_creator(calculation_string)
